...
import {
    ...
    updateDoc, addDoc, getDocs, writeBatch, deleteDoc, setDoc
} from 'firebase/firestore';
import { getFunctions, httpsCallable } from 'firebase/functions';
import { auth, db } from '../firebase/config';
// --- INICIO DE CAMBIOS ---
import { User, Client, WorkoutPlan, Exercise } from '../types';
// --- FIN DE CAMBIOS ---
...
export const AuthProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
    ...
    // --- INICIO DE CAMBIOS ---
    // Escuchador en tiempo real para el documento del usuario actual (si es cliente)
    // Esto asegura que la UI del cliente se actualice si el entrenador cambia el plan.
    useEffect(() => {
    ...
    }, [currentUser?.id]); // Depende del ID para re-suscribirse si el usuario cambia
    // --- FIN DE CAMBIOS ---
    ...
    const processUserLogin = async (firebaseUser: FirebaseUser) => {
        ...
        if (actionString) {
            ...
            if (actionData.action === 'subscribe') {
                ...
                try {
                    ...
                    alert("Hubo un error al conectar con el sistema de pago.");
        ...
        try {
            ...
            if (querySnapshot.empty) {
                ...
                alert("Tu cuenta no está registrada. Si eres un cliente, contacta a tu entrenador. Si eres un entrenador, completa la suscripción.");
                await firebaseSignout(auth);
                return;
            }
            ...
            if (userDoc.id !== firebaseUser.uid) {
                ...
                batch.set(newUserRef, profileDataForDB);
            ...
            alert(`Error de autenticación/DB. Se cerró la sesión. Detalle: ${error.code || error.message}`);
            await firebaseSignout(auth);
        }
    };
    ...
    const updateClientExercise = async (workoutId: string, exerciseIndex: number, updatedExercise: Exercise) => {
        ...
        // Hacemos una copia profunda del plan para evitar mutaciones directas del estado
        const newPlan = JSON.parse(JSON.stringify(client.plan));

        // Encontramos el índice del workout
        ...
        // Actualizamos el ejercicio específico
    ...
    const value = { currentUser, isAuthLoading, trainerClients, logout, updateClientPlan, addClient, removeClient, updateClientExercise };
