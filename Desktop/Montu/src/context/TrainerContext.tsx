    ...
    const updateTrainerProfile = async (data: {
        ...
        const promise = async () => {
            ...
            if (data.newProfileImage) {
                if (trainer.photoURL) { try { await deleteObject(ref(storage, trainer.photoURL)); } catch (e) { console.warn("No se pudo borrar la imagen antigua."); } }
            ...
            if (data.galleryImagesToDelete && data.galleryImagesToDelete.length > 0) {
                dataToUpdate.galleryImages = arrayRemove(...data.galleryImagesToDelete);
                data.galleryImagesToDelete.forEach(url => { try { deleteObject(ref(storage, url)); } catch (e) { console.warn("No se pudo borrar imagen de galería."); } });
            }
