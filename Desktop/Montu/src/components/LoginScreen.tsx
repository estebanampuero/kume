...
import { 
    GoogleAuthProvider, 
    // Usaremos signInWithPopup para evitar el problema de la URL de redirección en localhost
    signInWithPopup, 
    setPersistence, 
...
export const LoginScreen: React.FC<LoginScreenProps> = ({ onNavigateToLanding }) => {
    ...
    const handleGoogleSignIn = async () => {
        ...
        try {
            // 1. FORZAR PERSISTENCIA LOCAL: Asegura que la sesión se mantenga.
            await setPersistence(auth, browserLocalPersistence); 
            console.log("Persistencia establecida a LOCAL.");

            // 2. Inicia el proceso de autenticación con POPUP
            // Esto abrirá una ventana flotante y evitará la falla de redirección.
            const result = await signInWithPopup(auth, provider);
            
            console.log(`Login exitoso con Popup. Usuario: ${result.user.email}`);

            // Nota: Aquí no necesitamos hacer nada más. 
            // El onAuthStateChanged en AuthContext detectará la sesión establecida y 
            // ejecutará processUserLogin, lo cual es el comportamiento deseado.

        } catch (err: any) {
            console.error("Error al iniciar sesión con POPUP:", err);
            // Si esto falla, el problema es la API Key o las restricciones generales.
